    ...
    server: {
      ...
      proxy: {
        '/api/nutrition/analyze': {
          target: 'https://nutriplan-api.vercel.app',
          changeOrigin: true,
          headers: {
            'x-api-key': process.env.NUTRITION_API_KEY || process.env.VITE_NUTRITION_API_KEY || 'rMVu4aYBEzDZBvY5OHio1vk9tObxaIIxd0G4Ld0k',
          },
        },
      },
    },
...
/**
 * Dev server middleware executing Veyra Serverless API (/api/ai/chat) during `npm run dev`
 */
function veyraApiDevServerPlugin(): Plugin {
  return {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        req.on('end', async () => {
          try {
            ...
            console.error('[Veyra Dev API Error]:', err);
            res.statusCode = 500;
