...
export default function RecipeDetailsModal({ recipe, onClose }: RecipeDetailsModalProps) {
  ...
  useEffect(() => {
    let isMounted = true
    fetch(`/api/reviews?recipeId=${recipe.id}`).then(r => r.json()).then(data => {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      if (editingReviewId) {
        const res = await fetch(`/api/reviews?id=${editingReviewId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rating: ratingInput, text: reviewText.trim() }) })
        const data = await res.json()
        if (data.success) { setReviews(prev => prev.map(r => r.id === editingReviewId ? data.data : r)); addToast("Review updated!", "success") }
      } else {
        const res = await fetch("/api/reviews", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ recipeId: String(recipe.id), rating: ratingInput, text: reviewText.trim() }) })
        const data = await res.json()
