...
export default async function handleNotifications(req: Request) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const res = await VeyraApiRouter.addNotification(userId, body);
      return new Response(JSON.stringify(res), { status: res.success ? 200 : 400, headers: { 'Content-Type': 'application/json' } });
    }

    if (req.method === 'PATCH') {
      const body = await req.json().catch(() => ({}));
      const res = await VeyraApiRouter.markNotificationAsRead(userId, body.id);
      return new Response(JSON.stringify(res), { status: res.success ? 200 : 400, headers: { 'Content-Type': 'application/json' } });
    }
