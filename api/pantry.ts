...
export default async function handler(req: Request) {
  try {
    ...
    if (req.method === 'GET') {
      const apiRes = await VeyraApiRouter.getPantryItems(userId);
      return new Response(JSON.stringify(apiRes), { status: apiRes.success ? 200 : 400, headers: { 'Content-Type': 'application/json' } });
    }

    if (req.method === 'POST') {
      const body = await req.json();
      const apiRes = await VeyraApiRouter.addPantryItem(userId, body);
      return new Response(JSON.stringify(apiRes), { status: apiRes.success ? 200 : 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (req.method === 'PUT') {
      ...
      if (!itemId) {
        return new Response(JSON.stringify({ success: false, error: 'Missing pantry item ID' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
      }
      const apiRes = await VeyraApiRouter.updatePantryItem(userId, itemId, body);
      return new Response(JSON.stringify(apiRes), { status: apiRes.success ? 200 : 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    if (req.method === 'DELETE') {
      ...
      const apiRes = await VeyraApiRouter.deletePantryItem(userId, itemId);
      return new Response(JSON.stringify(apiRes), { status: apiRes.success ? 200 : 400, headers: { 'Content-Type': 'application/json' } });
    }
