...
import { getToken, setToken, apiFetch, API_BASE_URL } from "@/services/api/backendClient"
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // Sync backend state on mount & login -- supports both Postgres (Vercel) and Mongo (Express)
  useEffect(() => {
    if (isAuthenticated) {
  ...
  // On mount, try to restore session from JWT if API_BASE_URL is set (Express Mongo backend)
  useEffect(() => {
        ...
        .then(async (r) => {
          if (r.ok) {
            ...
            if (u && (u.email || u.firstName)) {
              // Map to UserProfile and restore
              const mapped = {
                name: u.name || `${u.firstName||''} ${u.lastName||''}`.trim(),
                email: u.email,
                age: u.age ?? 28,
                heightCm: u.height ?? 178,
                weightKg: u.weight ?? 80,
                targetWeightKg: u.targetWeight ?? 75,
                goal: u.wellnessGoal || u.goal || "Lose Weight",
                ...
                favoriteCuisines: u.favoriteFoods || u.favoriteCuisines || [],
                ...
                aiProactiveFrequency: "high" as const,
              };
              setUser(mapped as any);
              saveUserToStorage(mapped as any);
  ...
  const logout = () => {
    ...
    saveAuthState(false);
    setToken(null);
    try { localStorage.removeItem("veyra_token"); localStorage.removeItem("veyra_user_id"); } catch {}
